...
(function(){
  ...
  // 51.la 统计初始化结果（浏览器环境下失败才显示；Tauri 环境 skipped 不显示）
...
async function flushAndGetFullHistory(){
  if(gameHistory.length>0){
    ...
    // 统一 mv 为数组 [x,y,player,seed]（Rust TurnHistory.mv 为 Option<[u64;4]>，对象格式会导致反序列化失败）
    const merged=[...existing,...memOff].map(h=>{
      const m=h.mv;
      return {...h, mv: m ? (Array.isArray(m) ? [m[0],m[1],m[2],m[3]||0] : [m.x,m.y,m.player,m.seed||0]) : null};
    });
